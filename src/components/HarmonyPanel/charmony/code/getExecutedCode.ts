...
export function getExecutedCode(
  json: IntermediateJson
): CharmonyExecutedCode[] {
  const { explain, code } = json
  const locations = Object.entries(json.locations)
    .map(
      ([k, v]) => [Number.parseInt(k), v] as [number, IntermediateHarmonyCode]
    )
    .sort(([k1], [k2]) => k1 - k2)
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    firstEvaluatedPc =
      firstEvaluatedPc !== undefined
        ? Math.min(startingPc, firstEvaluatedPc)
        : startingPc
