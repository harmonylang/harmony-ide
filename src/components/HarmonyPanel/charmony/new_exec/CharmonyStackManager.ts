...
export default class CharmonyStackManager {
   ...
   */
  private setLocal(
    local: undefined | Record<string, IntermediateValueRepresentation>
  ) {
  ...
  private getFullStatus(
    tid: string,
    props: IntermediateMicroStep | IntermediateContext
  ) {
    const { mode, atomic, failure, interruptlevel, readonly, choose } = props
    const ongoingTrace = this.stackTrace[tid]
    const status = mode === undefined ? ongoingTrace.status : mode
    const augments: string[] = []
    const atomicLevel =
      atomic != null ? Number.parseInt(atomic) : ongoingTrace.atomic
    const readLevel =
      readonly != null ? Number.parseInt(readonly) : ongoingTrace.readonly
    const interruptLevelValue =
      interruptlevel != null
    ...
    const fullStatus =
      status + (augments.length > 0 ? ' ' + augments.join(' ') : '')
  ...
  setNewTid(newTid: thread_id, contexts: IntermediateContext[]) {
    Object.assign(
      this.contexts,
      Object.fromEntries(contexts.map((c) => [c.tid, c]))
    )
