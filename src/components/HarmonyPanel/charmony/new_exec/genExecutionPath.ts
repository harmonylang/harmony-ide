...
export function genExecutionPath(
  json: IntermediateJson
): Omit<CharmonyTopLevel, 'executedCode'> {
  ...
  macrosteps.forEach((macroStep, macroIdx) => {
    ...
    if (macroStep.microsteps.length > 0) {
      ...
      stackTraceManager.updateStack(
        tid,
        firstStep,
        1 === macroStep.microsteps.length
      )
    ...
    for (let i = 1; i < macroStep.microsteps.length; i++) {
      ...
      if (processStateChanged(microStep)) {
        ...
        stackTraceManager.updateStack(
          tid,
          microStep,
          i === macroStep.microsteps.length - 1
        )
        sliceDuration = 0
