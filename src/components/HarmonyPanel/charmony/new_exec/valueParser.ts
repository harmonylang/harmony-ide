import {
  IntermediateKeyValueRep,
  IntermediateValueRepresentation,
} from '../IntermediateJson'

export function parseIntermediateValueRep(
  v: IntermediateValueRepresentation
): unknown {
  const { value, type } = v
  switch (type) {
    case 'int':
    ...
    case 'pc':
      return value
    case 'address':
      return (value as IntermediateValueRepresentation[]).map((v) =>
        parseIntermediateValueRep(v)
      )
    case 'set':
      return (value as IntermediateValueRepresentation[]).map((v) =>
        parseIntermediateValueRep(v)
...
export function parseVariableSet(
  sharedValues: undefined | Record<string, IntermediateValueRepresentation>
